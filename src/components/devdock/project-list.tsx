...
import { PlusCircle, FileWarning, Search, LayoutGrid } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
         ...
         <div className="text-center py-16 px-4 border-2 border-dashed rounded-xl bg-card">
            <div className="flex justify-center items-center w-16 h-16 mx-auto bg-muted rounded-full mb-6 text-muted-foreground">
              {searchTerm ? <Search className="h-8 w-8" /> : <LayoutGrid className="h-8 w-8" />}
            </div>
            <h3 className="mt-4 text-2xl font-semibold">
              {searchTerm ? 'No Projects Found' : 'No Projects Yet'}
            </h3>
            <p className="mt-2 text-base text-muted-foreground max-w-md mx-auto">
              {searchTerm ? `Your search for "${searchTerm}" did not return any results. Try a different query.` : 'Get started by adding your first project to have quick access to its path, scripts, and more.'}
            </p>
            <div className="mt-6">
